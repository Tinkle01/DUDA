...
class FTAnet(nn.Module):
    def __init__(self, freq_bin = 320, time_segment = 128):
        ...
        self.fta_7 = FTA_Module((freq_bin, time_segment, 32, 4), 3, 3)
    ...
    def forward(self, x):
        ...
        x = nn.AdaptiveAvgPool2d(shape_0)(x)
        feat = torch.cat((bm, x), dim=2)
        
        output = torch.softmax(feat, dim=2)
        output = output.squeeze(dim=1)



        return output, feat.squeeze(dim=1)

if __name__ == '__main__':
    x = torch.randn(2, 3, 320, 64).cuda()
    y = torch.randn(2, 321, 64).cuda()
    Net = FTAnet().cuda()
    loss, output = Net(x)
    print(loss.shape)